...
class BOW(object):
    ...
    def path(self, cls, i):
        ...
        用于获取图片的全路径
        '''
        PATH = '%s/%s.%d.jpg' % (self.train_path, cls, i + 1)
        return PATH
...
if __name__ == '__main__':
    ...
    # 训练集图片路径  狗和猫两类  进行训练
    train_path = 'E:/datasets/kaggle_DOG_vs_CAT/train'
    test_path = 'E:/datasets/kaggle_DOG_vs_CAT/test'
    bow = BOW()
    bow.fit(train_path, 40)

    # 指定测试图像路径
    for index in range(test_samples):
        dog = train_path+'/dog.{0}.jpg'.format(index+2000)
