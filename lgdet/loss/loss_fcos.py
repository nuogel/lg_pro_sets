...
class GenTargets(nn.Module):
    ...
    def _gen_level_targets(self, out, gt_boxes, classes, stride, limit_range, sample_radiu_ratio=1.5):
        ...
        reg_targets = ltrb_off[(torch.zeros_like(areas).scatter_(-1, areas_min_ind.unsqueeze(dim=-1), 1)).to(torch.BoolTensor())]  # [batch_size*h*w,4]
        ...
        cls_targets = classes[(torch.zeros_like(areas).scatter_(-1, areas_min_ind.unsqueeze(dim=-1), 1)).to(torch.BoolTensor())]
...
class FCOSLOSS(nn.Module):
    ...
    def Loss_Call(self, predictions, targets, kwargs):
        ...
        gt_box, gt_cls = self.reshape_targets(B, targets)
        cls_targets, cnt_targets, reg_targets = self.make_target((predictions, gt_box, gt_cls))
