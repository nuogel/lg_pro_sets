...
class SRFBN(nn.Module):
    def __init__(self, cfg):
        ...
        self.num_groups = 6
        self.cfg.TRAIN.UPSCALE_FACTOR = self.cfg.TRAIN.UPSCALE_FACTOR

        if self.cfg.TRAIN.UPSCALE_FACTOR == 2:
            stride = 2
            padding = 2
            kernel_size = 6
        elif self.cfg.TRAIN.UPSCALE_FACTOR == 3:
            stride = 3
            padding = 2
            kernel_size = 7
        elif self.cfg.TRAIN.UPSCALE_FACTOR == 4:
            stride = 4
            padding = 2
            kernel_size = 8
        elif self.cfg.TRAIN.UPSCALE_FACTOR == 8:
            stride = 8
            padding = 2
            kernel_size = 12
        ...
        self.conv_in = ConvBlock(in_channels, 4 * self.num_features, kernel_size=3, act_type=act_type, norm_type=norm_type)
        self.feat_in = ConvBlock(4 * self.num_features, self.num_features, kernel_size=1, act_type=act_type, norm_type=norm_type)

        # basic block
        self.block = FeedbackBlock(self.num_features, self.num_groups, self.cfg.TRAIN.UPSCALE_FACTOR, act_type, norm_type)

        # reconstruction block
        # uncomment for pytorch 0.4.0
        # self.upsample = nn.Upsample(scale_factor=self.cfg.TRAIN.UPSCALE_FACTOR, mode='bilinear')

        self.out = DeconvBlock(self.num_features, self.num_features, kernel_size=kernel_size, stride=stride, padding=padding, act_type='prelu', norm_type=norm_type)
        self.conv_out = ConvBlock(self.num_features, out_channels, kernel_size=3, act_type=None, norm_type=norm_type)
    ...
    def forward(self, **args):
        ...
        inter_res = nn.functional.interpolate(x, scale_factor=self.cfg.TRAIN.UPSCALE_FACTOR, mode='bilinear', align_corners=False)
        ...
        for _ in range(self.num_steps):
            ...
            h = self.add_mean(h)
            outs.append(h)
        # out = outs[0].permute(0, 2, 3, 1)
        return outs[0]  # return output of every timesteps
