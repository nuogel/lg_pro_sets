...
class EFFICIENTDET(nn.Module):
    def __init__(self, cfg):
        ...
        self.neck = BIFPN(in_channels=self.backbone.get_list_features()[-5:],
                          ...
                          stack=D_bifpn,
                          num_outs=5)
        self.header = RetinaHead(num_classes=num_classes, in_channels=W_bifpn, num_anchors=3 * (len(scales)))
        self.priorbox = Anchors(pyramid_levels=pyramid_levels, scales=scales)
    ...
    def forward(self, **args):
        ...
        anchors_xywh = self.priorbox.forward(args['input_x']).to(self.device)
