...
class LgTransformer:
    ...
    def random_affine(self, img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, border=0):
        ...
        if n:
            ...
            i = (w > 2) & (h > 2) & (area / (area0 * s + 1e-16) > 0.01) & (ar < 20)
    ...
    def augment_hsv(self, im, label, data_info, hgain=0.05, sgain=0.5, vgain=0.5):
