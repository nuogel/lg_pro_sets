...
class EfficientDetLoss():
    ...
    def Loss_Call(self, predicted, dataset, losstype=None):
        ...
        classifications, regressions, anchors = predicted
        ...
        for j in range(batch_size):
            ...
            bbox_annotation = torch.Tensor(annotations[j]).to(anchors.device)
            ...
            IoU = calc_iou(anchors[0, :, :], bbox_annotation[:, 1:])  # num_anchors x num_annotations
            ...
            if positive_indices.sum() > 0:
                ...
                targets = targets / torch.Tensor([[0.1, 0.1, 0.2, 0.2]]).to(anchors.device)
        ...
        #     bce = bceloss(classification, targets)
        #     # bce = -(targets * torch.log(classification) + (1.0 - targets) * torch.log(1.0 - classification))
        #
        #     # cls_loss = focal_weight * torch.pow(bce, gamma)
        #     cls_loss = focal_weight * bce
        ...
        loc_loss = torch.stack(regression_losses).mean(dim=0, keepdim=True)
