...
def get_layers(model, filter_regexp):
    ...
    m_named_p = model.named_parameters()
    # all_layers = map(itemgetter(0), m_named_p)
    all_layers = []
    for name, para in m_named_p:
        if "bias" not in name:
            name = name.replace(".weight", "")
            name = name.replace(".weight_orig", "")
    ...
    out = list(filter(r.match, all_layers))
    return out
