...
class SiameseTracker:
    def get_subwindow(self, im, pos, model_sz, original_sz, avg_chans):
        ...
        print('context_xyxy:', context_xmin,context_xmax,context_ymin,context_ymax)
        r, c, k = im.shape
...
class MFTrackerORT(SiameseTracker):
    ...
    def init_track_net(self):
        """使用设置的参数初始化tracker网络
        """
        self.ort_session = onnxruntime.InferenceSession(self.model_path, providers=self.providers)
    ...
    def _convert_score(self, score):
        ...
        score_soft = F.softmax(score, dim=1).data[:, 1].cpu().numpy()
        return score_soft

    def _convert_bbox(self, delta, anchor):
        delta = torch.Tensor(delta)
        delta = delta.permute(1, 2, 3, 0).contiguous().view(4, -1)
        delta = delta.data.cpu().numpy()  #[4, 3125]
    ...
    def track(self, image, info: dict = None):
        ...
        z_crop = self.to_numpy(self.z_crop)
        x_crop = self.to_numpy(x_crop)

        # print("self.z_crop",self.z_crop)
        # print("x_crop", x_crop)
        rknn_zcrop = np.load('rknn_input_zcrop.npy')
        rknn_xcrop = np.load('rknn_input_xcrop.npy')
        rknn_output0 = np.load('rknn_input_outpus0.npy')

        # compute ONNX Runtime output prediction
        ort_inputs = {'input.1':z_crop , 'input.140':x_crop }

        ort_outs = self.ort_session.run(None, ort_inputs)
        # a = rknn_output0 - ort_outs[0]
        pred_bbox = self._convert_bbox(ort_outs[1],self.anchors)
        score = self._convert_score(ort_outs[0])
        best_idx_debug = np.argmax(score)
        print("debug:best id",best_idx_debug," best bbox:",pred_bbox[:,best_idx_debug], "best score:",score[best_idx_debug])
        ...
        print("best bbox:",bbox, "best score:",pscore[best_idx])
...
if __name__ == '__main__':
    print("测试")
    model_path = "/home/luogeng/code/lg_pro_sets/saved/checkpoint/siamrpn_onnx_ized.onnx"
    ...
    for frame in get_frames(Tracker.video_name):
        ...
        if first_frame:
            ...
            frame_id += 1
            # cv2.imwrite(f'outputs/{frame_id}_rpn.jpg', frame)
