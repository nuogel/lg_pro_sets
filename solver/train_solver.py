...
class Solver:
    ...
    def _train_an_epoch(self, epoch, optimizer, scheduler):
        ...
        self.Model.train()
        self.LOGGER.debug('>' * 30 + '[TRAIN] Model:%s,   Epoch: %s,   Learning Rate: %s', self.cfg.TRAIN.MODEL, epoch,
        ...
        self.LOGGER.debug('[TRAIN] Summary: Epoch: %s, average total loss: %s', epoch,
    ...
    def _test_an_epoch(self, epoch):
        ...
        for step, train_data in enumerate(Pbar):
            ...
            _timer.time_end()
            info = '[EVALUATE] Epoch-Step:%3d-%4d/%4d, Time Step/Total-%s/%s' % (
                epoch, step, len(self.testDataloader), _timer.diff, _timer.from_begin)
            self.LOGGER.debug(info)
            Pbar.set_description(info)
        ...
        self.save_parameter.tbX_write(w_dict)
        self.LOGGER.debug('[EVALUATE] Summary: Epoch: %s, total_score: %s, other_score: %s', epoch, str(main_score),
                          str(item_score))
